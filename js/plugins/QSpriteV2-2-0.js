...
(function() {
  ...
  _identifier = _identifier.replace('(', '\\(').replace(')', '\\)');
  _identifier = _identifier.replace('[', '\\[').replace(']', '\\]');
  _identifier = _identifier.replace('{config}', '(.+)?');
  _identifier = new RegExp(_identifier);
  var _identifier2  = '%2{config}[{pose}]';
  _identifier2 = _identifier2.replace('(', '\\(').replace(')', '\\)');
  _identifier2 = _identifier2.replace('[', '\\[').replace(']', '\\]');
  _identifier2 = _identifier2.replace('{pose}', '(?:.+)?');
  _identifier2 = _identifier2.replace('{config}', '(.+)?');
  _identifier2 = new RegExp(_identifier2);
  var _identifier3 = '%2{config}[{pose}]';
  _identifier3 = _identifier3.replace('(', '\\(').replace(')', '\\)');
  _identifier3 = _identifier3.replace('[', '\\[').replace(']', '\\]');
  _identifier3 = _identifier3.replace('{pose}', ')(.+)?(');
  _identifier3 = _identifier3.replace('{config}', '(?:.+)?');
  _identifier3 = '(' + _identifier3 + ')';
  _identifier3 = new RegExp(_identifier3);
  ...
  var _useNewAdjust = _params['Use New Adjust'].toLowerCase() === 'true';
  var _hasQMovement = Imported.Quasi_Movement || Imported.QMovement;

  QPlus.request('data/SpriteAnim.json')
  ...
  QSprite.format = function(config, dir) {
    if (config.formated) {
      return config.formated;
    }
    var format = config.format;
    var diags = config.diagonals;
    ...
    if (!diags) {
      ...
      if (format === 'A') {
        pattern = [2, 1, 4, 3, 6, 7, 8, 9];
      }
      if (format === 'B') {
        pattern = [1, 2, 3, 4, 7, 6, 9, 8];
      }
      if (format === 'C') {
        pattern = [2, 4, 6, 8, 1, 3, 7, 9];
      }
      if (format === 'D') {
        pattern = [1, 3, 7, 9, 2, 4, 6, 8,];
      }
    }
    config.formated = {};
    for (var i = 0; i < pattern.length; i++) {
      config.formated[pattern[i]] = i * frames;
    }
    return config.formated;
  };

  QSprite.getPatternX = function(config, character) {
    var format = this.format(config);
    var diags = config.diagonals;
    var dir = character._direction;
    if (diags && _hasQMovement && character.isDiagonal()) {
      dir = character.isDiagonal();
    }
    var cols = config.frames;
    if (config.format === 'A' || config.format === 'B') {
      if (diags) cols *= 2;
    }
    var i = format[dir] + character._pattern;
    return i % cols;
  };

  QSprite.getPatternY = function(config, character) {
    var format = this.format(config);
    var diags = config.diagonals;
    var dir = character._direction;
    if (diags && _hasQMovement && character.isDiagonal()) {
  ...
  QSprite.getPatternW = function(config, width) {
    var format = config.format;
    var w = width / config.frames;
    if (format === 'A' || format === 'B') {
      if (config.diagonals) w /= 2;
    }
    return w;
  };

  QSprite.getPatternH = function(config, height) {
    var format = config.format;
    var h = height / 4;
    if (format === 'C' || format === 'D') {
      if (config.diagonals) h /= 2;
    }
    return h;
  };
  ...
  Game_CharacterBase.prototype.animationWait = function() {
    if (this.qSprite()) {
      var isV2 = this.qSpriteV2();
      var pose;
      if (isV2) {
        pose = isV2;
      } else {
        pose = this.qSprite().poses[this._pose] || {};
      }
      if (pose.speed) {
        if (pose.adjust) {
          if (_useNewAdjust) {
  ...
  Game_CharacterBase.prototype.calcPoseWait = function() {
    ...
    var frames;
    if (isV2) {
      frames = isV2.frames || 0;
    } else {
      frames = this.qSprite().poses[this._pose].pattern.length;
    }
  ...
  Game_CharacterBase.prototype.updatePose = function(wasMoving) {
    ...
    if (this._pose === '') this.setPose('default');
  };
  ...
  Game_CharacterBase.prototype.updateIdlePose = function(dir, diag) {
    ...
    this.setPose(this._idlePose + dir);
  ...
  Game_CharacterBase.prototype.hasPose = function(pose) {
    if (this.qSprite()) {
      var isV2 = this.qSpriteV2();
      if (isV2) {
        return this.qSprite().poses.hasOwnProperty(pose.slice(0, -1));
      } else {
  ...
  Game_CharacterBase.prototype.setPose = function(pose) {
    var isV2 = this.qSpriteV2();
    if (isV2 && (pose !== '' || poseName !== 'default')) {
      var poseName = pose.slice(0, -1);
      var newImg = this._characterName.replace(_identifier3, '$1' + poseName + '$3');
      this._characterName = newImg;
    }
    this._pose = pose;
  };
  ...
  Game_CharacterBase.prototype.qSpriteV2 = function() {
    var config = this.qSprite();
    if (!config || !config.isV2) return false;
    return config.poses[this._pose.slice(0, -1)] || {};
  };
  ...
  Sprite_Character.prototype.updateBitmap = function() {
    if (this.isImageChanged()) {
      ...
      var qSprite = this._character.qSpriteV2() || this._character.qSprite();
      if (qSprite) {
  ...
  Sprite_Character.prototype.characterPatternX = function() {
    if (this._character.isQCharacter()) {
      var isV2 = this._character.qSpriteV2();
      if (isV2) {
        return QSprite.getPatternX(isV2, this._character);
      }
  ...
  Sprite_Character.prototype.characterPatternY = function() {
    if (this._character.isQCharacter()) {
      var isV2 = this._character.qSpriteV2();
      if (isV2) {
        return QSprite.getPatternY(isV2, this._character);
      }
      var pose = this._character.qSprite().poses[this._character._pose];
      if (!pose) return 0;
      var pattern = pose.pattern;
  ...
  Sprite_Character.prototype.patternWidth = function() {
    if (this._character.isQCharacter()) {
      var isV2 = this._character.qSpriteV2();
      if (isV2) {
        return QSprite.getPatternW(isV2, this.bitmap.width);
      }
  ...
  Sprite_Character.prototype.patternHeight = function() {
    if (this._character.isQCharacter()) {
      var isV2 = this._character.qSpriteV2();
      if (isV2) {
        return QSprite.getPatternH(isV2, this.bitmap.height);
      }
